...
interface IntentClassification {
  ...
  selected_indices?: number[];  // For "1&2" style selection
  filter_criteria?: string;     // For "Ireland only" style filtering
  explanation: string;
}
...
export async function onRequestPost(context: { request: Request; env: Env; params: { id: string } }): Promise<Response> {
  ...
  try {
    ...
    if (trip.status === 'researching') {
      ...
      const destinationsList = researchDestinations.map((d, i) =>
        `${i + 1}. ${d.name} (${d.geographic_context || 'location'})`
      ).join('\n');
...
Respond with ONLY valid JSON:
{
  ...
  "explanation": "brief explanation of classification"
      ...
      if (classification.intent === 'confirm') {
        ...
        let refinedDestinations: Destination[] = [...researchDestinations];
