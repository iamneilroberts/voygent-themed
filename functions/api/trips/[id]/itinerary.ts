...
import { Env, createDatabaseClient, TripOption } from '../../../lib/db';
...
export async function onRequestPost(context: { request: Request; env: Env; params: { id: string } }): Promise<Response> {
  ...
  try {
    const body = await context.request.json() as ItineraryRequest;
    const { option_index } = body;

    if (option_index === undefined || option_index === null) {
      return new Response(
        JSON.stringify({ error: 'option_index is required' }),
    ...
    // Parse options
    const options = trip.options_json ? JSON.parse(trip.options_json) as TripOption[] : [];
    if (options.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No trip options available' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!option) {
      return new Response(
        JSON.stringify({ error: `Option ${option_index} not found` }),
        { status: 404, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    const destinations = trip.confirmed_destinations
      ? JSON.parse(trip.confirmed_destinations) as string[]
      : [];
    ...
    const updatedOption = await tripBuilder.generateDailyItinerary(
      tripId,
      option,
      destinations,
      tripDays
    ...
    const updatedOptions = options.map(o =>
      o.option_index === option_index ? updatedOption : o
    );
