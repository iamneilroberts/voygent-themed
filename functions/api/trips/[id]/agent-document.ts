...
import { Env, createDatabaseClient, TripOption, TripPreferences, ChatMessage, DayItinerary } from '../../../lib/db';
...
export async function onRequestGet(context: { request: Request; env: Env; params: { id: string } }): Promise<Response> {
  ...
  try {
    ...
    // Parse all trip data
    const chatHistory: ChatMessage[] = trip.chat_history ? JSON.parse(trip.chat_history) : [];
    const preferences: TripPreferences = trip.preferences_json ? JSON.parse(trip.preferences_json) : {};
    const userContact: UserContact | null = trip.user_contact_json ? JSON.parse(trip.user_contact_json) : null;
    ...
    let selectedOption: TripOption | null = null;
    ...
    const html = generateAgentDocument(
      tripId,
      templateName,
 ...
 */
function generateAgentDocument(
  tripId: string,
  templateName: string,
...
function generatePriorityBox(status: string): string {
...
function generateClientSection(userContact: UserContact | null, travelers: TravelerInfo[], specialRequests: string | null): string {
  if (!userContact) {
    return `
    <section>
...
function generateRequestSection(templateName: string, templateDescription: string, initialMessage: string, preferences: TripPreferences): string {
  const prefItems = [];

  if (preferences.duration) prefItems.push({ label: 'Trip Duration', value: preferences.duration });
  if (preferences.departure_date) prefItems.push({ label: 'Departure Date', value: preferences.departure_date });
  if (preferences.departure_airport) prefItems.push({ label: 'Departure Airport', value: preferences.departure_airport });
  if (preferences.travelers_adults) prefItems.push({ label: 'Adults', value: String(preferences.travelers_adults) });
  if (preferences.travelers_children) prefItems.push({ label: 'Children', value: String(preferences.travelers_children) });
  if (preferences.luxury_level) prefItems.push({ label: 'Comfort Level', value: preferences.luxury_level });
  if (preferences.activity_level) prefItems.push({ label: 'Activity Level', value: preferences.activity_level });
...
function generateDestinationsSection(destinations: string[], researchDestinations: any[], researchSummary: any): string {
...
function generateBookingsSection(option: TripOption): string {
...
function generateCostSection(option: TripOption | null, aiCost: number, apiCost: number): string {
  if (!option) {
    return `
    <section>
...
function generateNextStepsSection(status: string): string {
  return `
  <section>
