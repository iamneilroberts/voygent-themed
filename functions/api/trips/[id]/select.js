...
export async function onRequestPost(context) {
    ...
    try {
        ...
        if (!body.option_index || body.option_index < 1) {
            return new Response(JSON.stringify({ error: 'option_index must be a positive integer (1-based)' }), {
                status: 400,
        ...
        return new Response(JSON.stringify({ error: 'Failed to select trip option' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' },
        });
    }
 ...
 */
function generateDetailedItinerary(option) {
    const itinerary = [];
