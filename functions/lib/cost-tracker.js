...
export class CostTracker {
     ...
     */
    async trackAICost(provider, model, inputTokens, outputTokens, costPer1kInputTokens, costPer1kOutputTokens) {
        ...
        this.logger.debug(`AI cost tracked: ${provider}/${model} - ${inputTokens + outputTokens} tokens = $${totalCost.toFixed(4)}`);
        return totalCost;
    }
    /**
     ...
     */
    async trackAPICost(api, endpoint, costUsd) {
        const cost = {
            api,
            endpoint,
            costUsd,
 ...
 */
export function createCostTracker(db, logger, tripId) {
