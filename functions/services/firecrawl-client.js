...
export class FirecrawlClient {
     ...
     */
    async scrape(url, options = {}, costTracker, telemetryCtx) {
     ...
     */
    async scrapeMultiple(urls, options = {}, costTracker, concurrency = 3, telemetryCtx) {
        ...
        for (let i = 0; i < urls.length; i += concurrency) {
            ...
            const batchResults = await Promise.all(batch.map(url => this.scrape(url, options, costTracker, telemetryCtx)));
     ...
     */
    async search(query, options = {}, costTracker, telemetryCtx) {
        const startTime = Date.now();
     ...
     */
    async findBookingUrl(itemName, city, type, costTracker, telemetryCtx) {
     ...
     */
    static extractSummary(markdown, maxLength = 2000) {
        if (!markdown)
            return '';
 ...
 */
export function createFirecrawlClient(env, logger) {
