...
import { Env, DatabaseClient, TripOption, TripPreferences, TransportationRecommendation, TransportSegment } from '../lib/db';
...
export class TransportationService {
   ...
   */
  async generateRecommendation(request: TransportationRequest): Promise<TransportationRecommendation> {
   ...
   */
  private analyzeDrivingSide(destinations: string[], homeCountry?: string): {
    requiresSideSwitch: boolean;
    destinationSide: 'left' | 'right' | 'mixed';
    warning?: string;
  } {
    const home = homeCountry || 'US'; // Default to US

    // Check if home country drives on the left
    const homeIsLeft = LEFT_DRIVING_COUNTRIES.some(c =>
      home.toLowerCase().includes(c.toLowerCase())
    );

    // Check destinations
    const destSides = destinations.map(dest => {
      const isLeft = LEFT_DRIVING_COUNTRIES.some(c =>
    ...
    let destinationSide: 'left' | 'right' | 'mixed';
    if (hasLeft && hasRight) {
   ...
   */
  private buildTravelerProfile(preferences: TripPreferences): {
  ...
  private async generateAIRecommendation(
    ...
    destinations: string[],
    preferences: TripPreferences,
    drivingSideAnalysis: { requiresSideSwitch: boolean; destinationSide: string; warning?: string },
    travelerProfile: { totalTravelers: number; hasChildren: boolean; hasSeniors: boolean; ageRange: string; drivingComfort: string },
...
Respond with ONLY valid JSON:
{
  ...
  "estimated_cost_usd": 500
    ...
    try {
      ...
      if (jsonMatch) {
        const recommendation = JSON.parse(jsonMatch[0]) as TransportationRecommendation;
  ...
  private generateFallbackRecommendation(
    destinations: string[],
    travelerProfile: { totalTravelers: number; hasChildren: boolean; hasSeniors: boolean; drivingComfort: string },
    drivingSideAnalysis: { requiresSideSwitch: boolean; warning?: string }
  ): TransportationRecommendation {
    let primaryMode: TransportationRecommendation['primary_mode'] = 'mixed';
    let reasoning = '';
 ...
 */
export function createTransportationService(env: Env, db: DatabaseClient, logger: Logger): TransportationService {
