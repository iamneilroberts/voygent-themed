...
import { Env, DatabaseClient, TripTemplate, TripOption, TripPreferences, Hotel, Tour, Flight } from '../lib/db';
...
import { AmadeusClient, createAmadeusClient, FlightOffer, HotelOffer } from './amadeus-client';
import { ViatorClient, createViatorClient, TourProduct } from './viator-client';
...
export interface TripBuildRequest {
  tripId: string;
...
export class TripBuilderService {
  ...
  private currentTripId: string | null = null;  // For telemetry context
  ...
  async buildTripOptions(request: TripBuildRequest): Promise<TripBuildResult> {
    ...
    this.currentModelId = (preferences as any)?.ai_model || undefined;
    ...
    try {
      ...
      const options = await this.generateTripOptions(
        template,
   ...
   */
  private async enrichBookingUrls(
    options: TripOption[],
    costTracker: CostTracker,
    tripId: string
  ): Promise<TripOption[]> {
    ...
    const hotelUrlCache = new Map<string, string>();
    const tourUrlCache = new Map<string, string>();

    const enrichedOptions: TripOption[] = [];
    ...
    for (const option of options) {
      ...
      for (const hotel of (option.hotels || [])) {
        ...
        const url = await this.firecrawlClient.findBookingUrl(
          hotel.name,
          hotel.city,
          'hotel',
          costTracker,
   ...
   */
  private async generateTripOptions(
    template: TripTemplate,
    destinations: string[],
    preferences: TripPreferences,
    flights: FlightOffer[],
...
[
  {
    ...
    try {
      ...
      try {
        options = JSON.parse(repairedJSON) as TripOption[];
      } catch (parseError) {
...
[
   ...
   */
  private buildBookingContext(flights: FlightOffer[], hotels: HotelOffer[], tours: TourProduct[]): {
    flights: string;
    hotels: string;
    tours: string;
  } {
   ...
   */
  private getTravelClass(luxuryLevel?: string): 'ECONOMY' | 'PREMIUM_ECONOMY' | 'BUSINESS' | 'FIRST' {
    switch (luxuryLevel) {
      case 'Luxury':
        return 'BUSINESS';
      case 'Comfort':
   ...
   */
  private getHotelRatings(luxuryLevel?: string): string[] {
    switch (luxuryLevel) {
      case 'Luxury':
        return ['4', '5'];
      case 'Comfort':
   ...
   */
  private getReturnDate(departureDate: string, duration: string): string {
 ...
 */
export function createTripBuilderService(env: Env, db: DatabaseClient, logger: Logger): TripBuilderService {
